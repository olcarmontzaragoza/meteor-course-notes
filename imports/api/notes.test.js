...
beforeEach(function() {
    Notes.remove({});
    Notes.insert(notesOne);
    Notes.insert(notesTwo);
});
...
it ('should returns a users note', function() {
  const res = Meteor.server.publish_handlers.apply({ userId: noteOne.userId });
...
it ('should return zero notes for user that has no notes', function() {
  const res = Meteor.server.publish_handlers.apply({ userId: 'randomid' });
  const notes = res.fetch();
